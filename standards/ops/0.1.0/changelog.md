...
- Added `observation_unit` to `uns` (REQUIRED). A list of strings declaring which column(s) define each aggregation row. Column values concatenated with `|` must equal the corresponding `aggregate_id`.
...
- Added OPTIONAL `channel_combos` metadata on the `examples.zarr` root group's `zarr.json`: a per-panel array of `{name, display_channels?, priority?}` declaring which channels to display (and their order) per channel combination, plus the panel display order. `display_channels` is an ordered list of `omero.channels[*].label`s — omit ⇒ all channels (default); each listed label MUST be present in every leaf of its combination. Additive and optional — absent metadata falls back to viewer defaults.
